        ...
        if (target instanceof LivingEntity && target.level instanceof ServerLevel) {
            ...
            if (preciseImpactSpot != null) {
                ...
                if (preciseImpactSpot.y > headMinRange && preciseImpactSpot.y < headMaxRange) {
                    for (int i = 0; i < 5; i++) {
                        ((ServerLevel) target.level).sendParticles(ParticleTypes.DAMAGE_INDICATOR, preciseImpactSpot.x + Item.random.nextDouble() - 0.5d, preciseImpactSpot.y + Item.random.nextDouble() - 0.5d, preciseImpactSpot.z + Item.random.nextDouble() - 0.5d, 3, 0, 0, 0, 0);
                    }
