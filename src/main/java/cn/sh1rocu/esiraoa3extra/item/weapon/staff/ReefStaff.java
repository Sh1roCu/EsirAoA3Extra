...
public class ReefStaff extends BaseStaff<Boolean> {
    ...
    public void cast(Level world, ItemStack staff, LivingEntity caster, Boolean args) {
        float velocityX = -Mth.sin(caster.yRot * (float) Math.PI / 180f) * Mth.cos(caster.xRot * (float) Math.PI / 180f);
        float velocityY = -Mth.sin(caster.xRot * (float) Math.PI / 180f);
        float velocityZ = Mth.cos(caster.yRot * (float) Math.PI / 180f) * Mth.cos(caster.xRot * (float) Math.PI / 180f);
