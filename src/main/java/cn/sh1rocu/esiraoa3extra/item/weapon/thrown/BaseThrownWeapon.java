...
public abstract class BaseThrownWeapon extends BaseGun {
    ...
    public void doImpactDamage(Entity target, LivingEntity shooter, BaseBullet bullet, float bulletDmgMultiplier) {
        if (target != null && dmg > 0.0f && DamageUtil.dealRangedDamage(target, shooter, bullet, (float) dmg * bulletDmgMultiplier * (1 + extraDmg) * (1 + (0.05f * (amplifierLevel + (10 * starLevel))))))
