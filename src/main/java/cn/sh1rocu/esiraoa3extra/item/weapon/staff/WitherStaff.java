...
public class WitherStaff extends BaseStaff<Object> {
    public WitherStaff(int durability) {
        super(durability);
    }
    ...
    public boolean doEntityImpact(BaseEnergyShot shot, Entity target, LivingEntity shooter) {
        ...
        if (DamageUtil.dealMagicDamage(shot, shooter, target, getDmg() * totalMod, false)) {
            EntityUtil.applyPotions(target, new EffectBuilder(Effects.WITHER, 60).level(4));
