...
public class LightSpark extends BaseBlaster {
    ...
    public void fire(ItemStack blaster, LivingEntity shooter) {
        LightSparkEntity lightSpark = new LightSparkEntity(shooter, this, 5);
        createEnergyShot(blaster, shooter, lightSpark);
    }
    ...
    public boolean doEntityImpact(BaseEnergyShot shot, Entity target, LivingEntity shooter) {
        if (!EntityUtil.isImmuneToSpecialAttacks(target, shooter)) {
            ...
            if (event.isCanceled()) {
                shooter.sendMessage(new TranslatableComponent("message.hybridcompat.light_spark"), Util.NIL_UUID);
                return false;
            }
