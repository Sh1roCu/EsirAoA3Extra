    ...
    public void doImpactDamage(Entity target, LivingEntity shooter, BaseBullet bullet, float bulletDmgMultiplier) {
        if (target != null) {
            ...
            if (DamageUtil.dealGunDamage(target, shooter, bullet, (float) getDamage() * bulletDmgMultiplier * shellMod * (1 + extraDmg) * (1 + (0.05f * (amplifierLevel + (10 * starLevel)))))) {
                ...
                if (target instanceof LivingEntity)
                    DamageUtil.doScaledKnockback((LivingEntity) target, shooter, ((float) getDamage() * bulletDmgMultiplier) * shellMod * (1 + extraDmg) * (1 + (0.05f * (amplifierLevel + (10 * starLevel)))) / 10f, shooter.getX() - target.getX(), shooter.getZ() - target.getZ());
